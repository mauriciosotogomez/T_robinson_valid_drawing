...
parser.add_argument("-f", "--example file",
                   dest="input_file", default='robinson.csv',
...
# for (i,j,k,l) in four_points :    
#     print(f"[{(i,j,k,l)}] : d({i},{j})+d({k},{l})={value(distance[i][j])+value(distance[k][l])}, d({i},{k})+d({j},{l})={value(distance[i][k]) + value(distance[j][l])}, d({i},{l})+d({j},{k})={value(distance[i][l]) + value(distance[j][k])}") 
