...
# Create the problem
problem = LpProblem("Has a line diagram Problem",LpMaximize)

# Define decision variables
distance = LpVariable.dicts("distance", (ROWS, COLS), cat='Continuous', lowBound=0)
min = LpVariable.dicts("min", (three_points), cat='Continuous',  lowBound=0)

# We define an objective function since none is needed
problem += lpSum([min[q] for q in three_points])
...
for (i,j,k) in three_points:
    ...
    problem += min[(i,j,k)] <= distance[i][j] + distance[j][k] - distance[i][k] 
    problem += min[(i,j,k)] <= distance[i][k] + distance[k][j] - distance[i][j] 
    problem += min[(i,j,k)] <= distance[k][i] + distance[i][j] - distance[k][j] 
